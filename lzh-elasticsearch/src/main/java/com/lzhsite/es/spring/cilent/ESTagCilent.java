...
public class ESTagCilent {
    ...
    public JSONObject getUserInfoList(String productid, String startdate, String stopdate) throws Exception {  
        ...
        SearchResponse response = esOperateSdk.esClient().prepareSearch("razor_cd")  
                .setTypes("clientdata")  
    ...
    public JSONObject getUserInfoListByTags(String productid, String tags) throws Exception {  
        ...
        {  
         ...
         SearchResponse response = esOperateSdk.esClient().prepareSearch("razor_tag")  
                 .setTypes("user_tag")  
                 .setQuery(boolQueryBuilder)  
                 //.addField("user_id")  
                 .setSize(20)  
    ...
    public String getUserDetailTags(String productid, String userid) throws Exception {  
        ...
        GetResponse usertag= esOperateSdk.esClient().prepareGet("razor_tag", "user_tag", userid + "|" + productid).get();  
    ...
    public JSONArray getUserDetail(String productid, String startdate, String stopdate,String userid) throws Exception {  
        ...
        {  
            ...
            GetResponse usinglog= esOperateSdk.esClient().prepareGet("razor_usinglog", "usinglog", userid + "|" + sessionid).get();  
            jsonObject.put("usinglog", usinglog.getSourceAsString());  
            //3.添本次session的event  
            GetResponse event= esOperateSdk.esClient().prepareGet("razor_event", "event", userid + "|" + sessionid).get();  
    ...
    public String getTagTop(String productid) throws Exception { 
    	...
    	// select app_tags.tag_name count(*) 
    	// from user_tag 
...
//        search.addFields("actId","userId"); //另一种写法
        ...
        BoolQueryBuilder boolQuery = QueryBuilders.boolQuery()
                ...
                .filter(QueryBuilders.queryStringQuery("+data.subjectName:* +schoolName:"+schoolName).defaultOperator(Operator.valueOf("AND")));
        //设置query
    ...
    public   void testTwoAggString() throws Exception{
        //构造search请求
        SearchRequestBuilder search=esOperateSdk.esClient().prepareSearch("userlog*").setTypes("logs");
        search.setQuery(QueryBuilders.queryStringQuery("@timestamp:[ "+new Date().getTime()
        +" TO "+DateUtils.addMins(new Date(),123).getTime()+"}"
        ));
        //一级分组字段
